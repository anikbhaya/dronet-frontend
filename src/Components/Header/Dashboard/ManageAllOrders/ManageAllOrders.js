...
const ManageAllOrders = () => {
    ...
    const handleUpdateStatus = (e) => {
        ...
        axios.post('https://hidden-anchorage-77198.herokuapp.com/updateStatus', { "status": status, "id": id })
    ...
    useEffect(() => {
        axios.post('https://hidden-anchorage-77198.herokuapp.com/getAllOrders', { "userEmail": user.email || NaN })
            .then(function (response) {
    ...
    return (
        <div>
            ...
            <h2 className="text-3xl font-bold mb-6"><span className="text-primary">Manage</span> All Orders</h2>
                                                ...
                                                <tr key={order._id}>
                                                    ...
                                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                        ...
                                                        < select  onChange={e => handleUpdateStatus(e)} defaultValue={order?.status} id={order?._id}
                                                            ...
                                                            className={`border-2  form-select block w-full p-2 font-semibold ${order?.status == 'Pending' ? 'text-yellow-500 border-yellow-500' : 'text-green-500 border-green-500'}`} >
                                                            <option className="text-yellow-500 font-semibold" value="Pending">Pending</option>
                                                            <option value="Approved" className="text-green-500 font-semibold">Approved</option>
                                                        </select >
                                                    </td>
                                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
